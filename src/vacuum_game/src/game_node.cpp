...
#include <vacuum_msgs/srv/reset_game.hpp>
...
public:
    GameNode() : rclcpp::Node("game_node") {
        ...
        vacuum_core::environment::Config ec{cfg_.map_path, cfg_.resolution};
        env_ = std::make_shared<vacuum_core::environment::Environment>(ec);
        logic_ = std::make_unique<vacuum_core::game::GameLogic>(cfg_, env_);
        ...
        game_state_pub_ = create_publisher<vacuum_msgs::msg::GameState>("/game_state", 10);
        ...
        collect_srv_ = create_service<vacuum_msgs::srv::CollectTrash>(
            "/collect_trash",
            [this](const std::shared_ptr<vacuum_msgs::srv::CollectTrash::Request> req,
                   std::shared_ptr<vacuum_msgs::srv::CollectTrash::Response> resp) {
                vacuum_core::geometry::Point2d pos{req->robot_pos.x, req->robot_pos.y};
        ...
        unload_srv_ = create_service<vacuum_msgs::srv::UnloadTrash>(
            "/unload_trash",
            [this](const std::shared_ptr<vacuum_msgs::srv::UnloadTrash::Request> req,
                   std::shared_ptr<vacuum_msgs::srv::UnloadTrash::Response> resp) {
                vacuum_core::geometry::Point2d pos{req->robot_pos.x, req->robot_pos.y};
        ...
        reset_srv_ = create_service<vacuum_msgs::srv::ResetGame>(
            "/reset_game",
            [this](const std::shared_ptr<vacuum_msgs::srv::ResetGame::Request>,
                   std::shared_ptr<vacuum_msgs::srv::ResetGame::Response> resp) {
                logic_ = std::make_unique<vacuum_core::game::GameLogic>(cfg_, env_);
                for (const auto& r : cfg_.robots) {
                    logic_->registerRobot(r.robot_id);
                }
...
private:
    ...
    void publishTrashMarkers(const vacuum_core::game::GameSnapshot& snap) {
        // Use the trash's own unique id as the marker id so RViz updates in
        // place. To handle removed trash, send explicit DELETE for ids that
        // were published last time but are no longer active. This avoids
        // DELETEALL (which collides with id=0 on the first ADD and triggers
        // RViz's "Duplicate Marker" warning).
        visualization_msgs::msg::MarkerArray arr;
    ...
    void publishGameState(const vacuum_core::game::GameSnapshot& snap) {
        vacuum_msgs::msg::GameState msg;
        msg.mode = cfg_.mode;
        ...
        for (const auto& [id, used] : snap.capacity_used) {
            vacuum_msgs::msg::RobotStatus rs;
            rs.robot_id = id;
    ...
    std::unique_ptr<vacuum_core::game::GameLogic> logic_;
    bool announced_{false};

    rclcpp::Publisher<nav_msgs::msg::OccupancyGrid>::SharedPtr map_pub_;
    rclcpp::Publisher<vacuum_msgs::msg::GameState>::SharedPtr game_state_pub_;
    ...
    rclcpp::Service<vacuum_msgs::srv::CollectTrash>::SharedPtr collect_srv_;
    rclcpp::Service<vacuum_msgs::srv::UnloadTrash>::SharedPtr unload_srv_;
    rclcpp::Service<vacuum_msgs::srv::ResetGame>::SharedPtr reset_srv_;

...
template <typename T>
T ConfigLoader::required(const YAML::Node& node, const std::string& key) {
    if (!node[key]) {
...
template <typename T>
T ConfigLoader::optional(const YAML::Node& node, const std::string& key, const T& fallback) {
...
world::Color ConfigLoader::parseColor(const YAML::Node& node, world::Color fallback) {
...
GameConfig ConfigLoader::parseNode(const YAML::Node& root) {
    if (!root["environment"]) {

...
public:
    LidarNode() : rclcpp::Node("lidar_node") {
        ...
        auto cfg = core::config::ConfigLoader::loadFromFile(config_path_);
        core::environment::Config ec{cfg.map_path, cfg.resolution};
        env_ = std::make_shared<core::environment::Environment>(ec);

        core::lidar::Config lc;
        lc.max_range = max_range_;
        lc.beam_count = beam_count_;
        lc.first_ray_angle = first_angle_;
        lc.last_ray_angle = last_angle_;
        lidar_ = std::make_unique<core::lidar::Lidar>(lc, env_);
...
private:
    void publishScan(const msgs::msg::RobotStatus& status) {
        core::geometry::RobotState state;
    ...
    std::unique_ptr<core::lidar::Lidar> lidar_;

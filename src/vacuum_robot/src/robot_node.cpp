...
#include <vacuum_core/robot/Robot.h>
...
#include <vacuum_msgs/msg/game_state.hpp>
#include <vacuum_msgs/srv/collect_trash.hpp>
#include <vacuum_msgs/srv/unload_trash.hpp>
...
private:
    ...
    void loadConfig() {
        ...
        cfg_ = vacuum_core::config::ConfigLoader::loadFromFile(config_path_);
    }

    void setupCollisionEnvironment() {
        vacuum_core::environment::Config env_cfg{
        ...
        for (const auto& o : cfg_.obstacles) {
            if (o.shape == "circle") {
                obstacles_.push_back(std::make_shared<vacuum_core::world::CircleObstacle>(
                    id++, o.center, o.radius));
            } else if (o.shape == "rectangle") {
                obstacles_.push_back(std::make_shared<vacuum_core::world::RectangleObstacle>(
    ...
    void setupRobot() {
        ...
        vacuum_core::robot::Robot::CollisionCb cb = nullptr;
        if (!ghost) {
            cb = [env_ptr, obstacles, robot_radius]
                 (vacuum_core::geometry::RobotState s) -> bool {
        ...
        robot_ = std::make_unique<vacuum_core::robot::Robot>(rcfg, cb, true);
    }
    ...
    void setupTopicsAndTimers() {
        ...
        cmd_sub_ = create_subscription<geometry_msgs::msg::Twist>(
            ...
            [this](const geometry_msgs::msg::Twist::SharedPtr msg) {
                vacuum_core::geometry::Twist t{msg->linear.x, msg->angular.z};
                robot_->setVelocity(t);
            });

        status_pub_ = create_publisher<vacuum_msgs::msg::RobotStatus>(ns + "/status", qos);
        path_pub_ = create_publisher<nav_msgs::msg::Path>(ns + "/path", qos);
        marker_pub_ = create_publisher<visualization_msgs::msg::MarkerArray>(
            ns + "/robot_marker", qos);

        game_state_sub_ = create_subscription<vacuum_msgs::msg::GameState>(
            "/game_state", qos,
            [this](const vacuum_msgs::msg::GameState::SharedPtr msg) {
                latest_game_state_ = *msg;
                have_game_state_ = true;
            });

        collect_client_ = create_client<vacuum_msgs::srv::CollectTrash>("/collect_trash");
        unload_client_ = create_client<vacuum_msgs::srv::UnloadTrash>("/unload_trash");
    ...
    void callCollect(const vacuum_core::geometry::RobotState& state) {
        if (!collect_client_->service_is_ready()) return;
        auto req = std::make_shared<vacuum_msgs::srv::CollectTrash::Request>();
        ...
        collect_client_->async_send_request(req,
            [this](rclcpp::Client<vacuum_msgs::srv::CollectTrash>::SharedFuture f) {
                auto resp = f.get();
    ...
    void callUnload(const vacuum_core::geometry::RobotState& state) {
        if (!unload_client_->service_is_ready()) return;
        auto req = std::make_shared<vacuum_msgs::srv::UnloadTrash::Request>();
        req->robot_id = robot_id_;
        req->robot_pos.x = state.x;
        req->robot_pos.y = state.y;
        call_in_flight_ = true;
        unload_client_->async_send_request(req,
            [this](rclcpp::Client<vacuum_msgs::srv::UnloadTrash>::SharedFuture f) {
                auto resp = f.get();
                last_capacity_used_ = resp->capacity_used;
                if (resp->success) {
                    RCLCPP_INFO(get_logger(),
    ...
    void publishRobotMarker(const vacuum_core::geometry::RobotState& state,
    ...
    void publishState() {
        ...
        vacuum_msgs::msg::RobotStatus status;
    ...
    vacuum_core::config::GameConfig cfg_;
    std::shared_ptr<vacuum_core::environment::Environment> env_;
    std::vector<std::shared_ptr<vacuum_core::world::Obstacle>> obstacles_;
    std::unique_ptr<vacuum_core::robot::Robot> robot_;

    rclcpp::Subscription<geometry_msgs::msg::Twist>::SharedPtr cmd_sub_;
    rclcpp::Subscription<vacuum_msgs::msg::GameState>::SharedPtr game_state_sub_;
    rclcpp::Publisher<vacuum_msgs::msg::RobotStatus>::SharedPtr status_pub_;
    rclcpp::Publisher<nav_msgs::msg::Path>::SharedPtr path_pub_;
    rclcpp::Publisher<visualization_msgs::msg::MarkerArray>::SharedPtr marker_pub_;
    rclcpp::Client<vacuum_msgs::srv::CollectTrash>::SharedPtr collect_client_;
    rclcpp::Client<vacuum_msgs::srv::UnloadTrash>::SharedPtr unload_client_;
    std::unique_ptr<tf2_ros::TransformBroadcaster> tf_broadcaster_;
    rclcpp::TimerBase::SharedPtr timer_;
    rclcpp::TimerBase::SharedPtr interaction_timer_;
    nav_msgs::msg::Path path_msg_;
    vacuum_msgs::msg::GameState latest_game_state_;
};
